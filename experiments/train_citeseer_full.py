...
def load_args():
    parser = argparse.ArgumentParser(
        description="Structure-Aware Transformer on CitationFull datasets",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
    )
    parser.add_argument("--seed", type=int, default=0, help="random seed")
    parser.add_argument(
        "--dataset", type=str, default="citeseer", help="name of dataset"
    )
...
def main():
    ...
    dataset_citeseer_full = CitationFull(
        root=data_path,
        name="CiteSeer",
        transform=T.Compose(
            [
    ...
    if args.only_topo:
        print('only topo')
        dataset_citeseer_full[0].x = torch.zeros_like(dataset_citeseer_full[0].x, dtype=float)

    n_tags = dataset_citeseer_full.num_node_features
    num_class = dataset_citeseer_full.num_classes
    args.num_class = num_class
    num_edge_features = 0

    parsed_dataset = GraphDataset(
        dataset_citeseer_full,
        degree=True,
        k_hop=args.k_hop,
        se=args.se,
    ...
    deg = torch.cat(
        [
            utils.degree(
                dataset_citeseer_full[i].edge_index[1], num_nodes=dataset_citeseer_full[i].num_nodes
            )
            for i in range(len(dataset_citeseer_full))
        ]
    )
    print("deg", deg)
    print("len deg", len(deg))
    train_mask = dataset_citeseer_full[0].train_mask
    val_mask = dataset_citeseer_full[0].val_mask
    test_mask = dataset_citeseer_full[0].test_mask
